...
@Table(name = "overseas_stock_ai_summary", uniqueConstraints = {
        @UniqueConstraint(columnNames = { "stockCode" })
})
public class OverseasStockAiSummary {
    ...
    private double lastSupplyScore; // 수급 다이버전스 감지용 (갱신 트리거) - 해외 주식은 추후 구현 예정
    private double lastForeignerZScore; // 수급 다이버전스 감지용 - 해외 주식 추후 구현 예정
