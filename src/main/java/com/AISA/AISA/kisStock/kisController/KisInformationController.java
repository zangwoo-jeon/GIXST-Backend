...
public class KisInformationController {
        ...
        @PostMapping("/suspended/refresh/{stockCode}")
        @Operation(summary = "특정 종목 거래정지 여부 갱신 (API -> DB)", description = "KIS API에서 최신 거래정지 여부 정보를 가져와 DB에 저장합니다.")
        public ResponseEntity<SuccessResponse<String>> refreshSuspensionStatus(@PathVariable String stockCode) {
                kisInformationService.updateStockSuspensionStatus(stockCode);
                return ResponseEntity.ok(new SuccessResponse<>(true, "거래정지 여부 갱신 성공 (" + stockCode + ")", null));
        }

        @PostMapping("/suspended/refresh/all")
        @Operation(summary = "전체 국내 주식 거래정지 여부 일괄 갱신", description = "모든 국내 주식 종목에 대해 KIS API를 통해 거래정지 여부를 일괄적으로 갱신합니다. (비동기)")
        public ResponseEntity<SuccessResponse<String>> refreshAllSuspensionStatuses() {
                new Thread(() -> kisInformationService.updateAllStockSuspensionStatus()).start();
                return ResponseEntity.ok(new SuccessResponse<>(true, "전체 종목 거래정지 여부 갱신 시작 (백그라운드 실행)",
                                "Started background task"));
        }
        ...
        public ResponseEntity<SuccessResponse<String>> initAllInvestorTrendsByDate(
                ...
                return ResponseEntity.ok(new SuccessResponse<>(true,
                                "전체 종목 수급 데이터 초기화 시작 (백그라운드, " + startDate + "~" + endDate + ", marketCode="
                                                + marketCode
        ...
        public ResponseEntity<SuccessResponse<String>> refreshInvestorTrend(
                        @PathVariable String stockCode,
                        @RequestParam(defaultValue = "UN") String marketCode) {
