    ...
    public void fetchAndSaveInvestorTrend(String stockCode, LocalDate startDate, LocalDate endDate, String marketCode) {
        ...
        while (currentDate.isAfter(targetDate)) {
            try {
                ...
                final String queryMarketCode = effectiveMarketCode[0]; // 람다 캡처용
                KisInvestorDailyResponse response = kisApiClient.fetch(token -> webClient.get()
                        .uri(uriBuilder -> uriBuilder
                                .path(url)
                                .queryParam("FID_COND_MRKT_DIV_CODE", queryMarketCode)
                                .queryParam("FID_INPUT_ISCD", stockCode)
                ...
                if (response == null || response.getOutput() == null
                    ...
                    log.warn("Empty response for {} on {} (marketCode={}). Moving to previous day.",
                            stockCode, queryDate, queryMarketCode);
                    currentDate = currentDate.minusDays(1);
                    consecutiveSkips++;

                    // UN으로 연속 빈 응답 시 J로 자동 전환 (NXT 미상장 종목 처리)
                    if (!fallbackAttempted && "UN".equals(marketCode) && consecutiveSkips >= FALLBACK_THRESHOLD) {
                        log.info("Switching marketCode from UN to J for {} after {} consecutive empty responses.",
                                stockCode, consecutiveSkips);
                        effectiveMarketCode[0] = "J";
                        fallbackAttempted = true;
                        currentDate = endDate; // J로 처음부터 재시도
                        consecutiveSkips = 0;
                        continue;
                    }

                    if (consecutiveSkips >= MAX_CONSECUTIVE_SKIPS) {
                        log.warn("Too many consecutive empty responses for {}. Stopping.", stockCode);
                        break;
                    }
                    continue;
                }
                ...
                if (!oldestDate.isBefore(currentDate)) {
                    // No backward progress: move one day back manually to avoid infinite loop
                    log.warn("No backward progress for {} at {}. Moving to previous day.", stockCode, currentDate);
                    currentDate = currentDate.minusDays(1);
                    if (++consecutiveSkips >= MAX_CONSECUTIVE_SKIPS) {
                        log.warn("Too many consecutive skips for {}. Stopping.", stockCode);
                        break;
                    }
                    continue;
                }
                ...
                log.error("Error fetching investor trend for {} on {}: {}", stockCode, currentDate, e.getMessage());
                currentDate = currentDate.minusDays(1);
                consecutiveSkips++;

                // UN으로 연속 오류 시 J로 자동 전환 (stck_bsop_date가 빈 문자열로 파싱 오류 발생하는 NXT 미상장 종목)
                if (!fallbackAttempted && "UN".equals(marketCode) && consecutiveSkips >= FALLBACK_THRESHOLD) {
                    log.info("Switching marketCode from UN to J for {} after {} consecutive errors.",
                            stockCode, consecutiveSkips);
                    effectiveMarketCode[0] = "J";
                    fallbackAttempted = true;
                    currentDate = endDate; // J로 처음부터 재시도
                ...
                if (consecutiveSkips >= MAX_CONSECUTIVE_SKIPS) {
                    log.warn("Too many consecutive errors for {}. Stopping.", stockCode);
                    break;
                }
            }
        }
